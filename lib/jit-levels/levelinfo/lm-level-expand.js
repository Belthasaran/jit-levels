 ...
 *   Default for locked ROMs: fail loud unless `unlock:true` or `lockPolicy:'ignore'|'unlock'`.
 */
function expandLayer1Map16Fingerprints(romBuffer, levelId, opts = {}) {
  const { prepareRomForExpand } = require('../lock/virtual-unlock');
  const prepared = prepareRomForExpand(romBuffer, opts);
  const rom = createRomFromBuffer(prepared.buffer);
  ...
  const fp = fingerprintFromGrid(grid);
  fp.rom_lock = prepared.detect.rom_lock;
  fp.unlock = prepared.unlockResult ? prepared.unlockResult.unlock : null;
  return fp;
}
